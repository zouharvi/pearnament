...
function check_unlock() {
  ...
  if (annotation_score && !response_log.every(r => r.score != null)) {
    $("#button_next").attr("disabled", "disabled")
    $("#button_next").val("Next 🚧")
    return
  }
...
async function display_next_payload(response: DataPayload) {
  ...
  for (let item_i = 0; item_i < data.length; item_i++) {
    ...
    let tgt_chars = no_tgt_char ? item.tgt : (contentToCharSpans(item.tgt, "tgt_char") + (annotation_error_spans ? ' <span class="tgt_char char_missing">[missing]</span>' : ""))
    let output_block = $(`
      ...
      ${annotation_score ? _slider_html(item_i) : ""}
      </div>
    `)
    ...
    if (!no_tgt_char && (annotation_error_spans || annotation_error_categories) && errorSpansToLoad.length > 0) {
