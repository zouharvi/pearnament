...
/**
 * Applies CSS class for error span severity highlighting
 */
export function applySpanSeverityClass(
    tgt_chars_objs: Array<CharData>,
    left_i: number,
    right_i: number,
...
/**
 * Sets up the category dropdowns for an existing span with pre-filled values
 */
export function setupCategoryDropdowns(
    toolbox: JQuery<HTMLElement>,
    error_span: ErrorSpan
): void {
    if (error_span.category) {
        const parts = error_span.category.split("/")
        const cat1 = parts[0]
        const cat2 = parts.length > 1 ? parts[1] : ""
        
        // Set first dropdown
        toolbox.find("select").eq(0).val(cat1)
        
        // Populate and set second dropdown
        if (cat1 && MQM_ERROR_CATEGORIES[cat1]) {
            const subcat_select = toolbox.find("select").eq(1)
            subcat_select.empty()
            for (const subcat of MQM_ERROR_CATEGORIES[cat1]) {
                subcat_select.append(`<option value="${subcat}">${subcat}</option>`)
            }
            if (cat1 === "Other") {
                subcat_select.prop("disabled", true)
